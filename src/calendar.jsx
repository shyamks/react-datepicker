...
export default class Calendar extends React.Component {
    ...
    handleDayClick = (day, event, monthSelectedIn) => this.props.onSelect(day, event, monthSelectedIn);
    ...
    changeMonthYear = (monthYear) => {
        this.setState(
            {
                date: setYear(setMonth(this.state.date, getMonth(monthYear)), getYear(monthYear)),
            },
    ...
    formatWeekday = (day, locale) => {
        ...
        return this.props.useWeekdaysShort ? getWeekdayShortInLocale(day, locale) : getWeekdayMinInLocale(day, locale);
    };
    ...
    renderPreviousButton = () => {
        ...
        const allPrevDaysDisabled = monthDisabledBefore(this.state.date, this.props);

        if (
            (!this.props.forceShowMonthNavigation && !this.props.showDisabledMonthNavigation && allPrevDaysDisabled) ||
            this.props.showTimeSelectOnly
        ) {
            return;
        }

        const classes = ["react-datepicker__navigation", "react-datepicker__navigation--previous"];
    ...
    renderNextButton = () => {
        ...
        if (
            (!this.props.forceShowMonthNavigation && !this.props.showDisabledMonthNavigation && allNextDaysDisabled) ||
            this.props.showTimeSelectOnly
        ) {
            return;
        }

        const classes = ["react-datepicker__navigation", "react-datepicker__navigation--next"];
        if (this.props.showTimeSelect) {
        ...
        return (
            <button type="button" className={classes.join(" ")} onClick={clickHandler}>
                {this.props.showMonthYearPicker ? this.props.nextYearButtonLabel : this.props.nextMonthButtonLabel}
            </button>
        );
    };
    ...
    renderCurrentMonth = (date = this.state.date) => {
        ...
        return <div className={classes.join(" ")}>{formatDate(date, this.props.dateFormat, this.props.locale)}</div>;
    };
    ...
    renderTodayButton = () => {
        ...
        return (
            <div className="react-datepicker__today-button" onClick={(e) => this.props.onSelect(getStartOfToday(), e)}>
    ...
    renderCustomHeader = ({ monthDate, i }) => {
        ...
        const prevMonthButtonDisabled = monthDisabledBefore(this.state.date, this.props);

        const nextMonthButtonDisabled = monthDisabledAfter(this.state.date, this.props);
    ...
    renderYearHeader = () => {
        return <div className="react-datepicker__header react-datepicker-year-header">{getYear(this.state.date)}</div>;
    };
    ...
    renderTimeSection = () => {
        if (this.props.showTimeSelect && (this.state.monthContainer || this.props.showTimeSelectOnly)) {
            return (
                <Time
                    selected={this.props.selected}
    ...
    renderInputTimeSection = () => {
        ...
        const timeString = `${addZero(time.getHours())}:${addZero(time.getMinutes())}`;
